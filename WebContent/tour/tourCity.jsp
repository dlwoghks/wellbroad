...
<body>
	...
	<c:set var="city" value="<%=new com.wellbroad.db.beans.tour.TourCityDTO()%>" />
		...
		<c:if test="${city.name!=null}">
			...
			<!-- 도시를 선택하면 추가되는 것 정보(이름,이미지,설명) -->
				...
				<div class="tour_image" align="center">
					<img src="../custom/image/city/${city.code_city}.png" width="1000">
				</div>
				<!-- 도시설명 -->
				<div class="tour_info">
					<div class="tour_description">${city.description }</div>
				</div>
			</div><!-- /.sub-container2 -->
		</c:if>
	</div><!-- /.sub-main -->
	...
	<div class="modal-dialog modal-80size modal-center" role="document">
		<div class="modal-content modal-80size">
			...
			<div class="modal-body" style="text-align: center;">
				<div id="map-canvas" style="width: 100%; height: 500px" title="도시 위치입니다."></div>
			</div>
			<!-- modal-body -->
...
function initMap() {
	var selectcity = $('#city option:selected').val();
 	...
 	// Geocoding *****************************************************
 	var address = 'phuket';  //선택하지 않았을 시에 보여질 화면
 	if(selectcity != null) address = selectcity; //도시를 선택했다면 해당 도시의 주소 검색
 	var marker = null;
 					...
 					function(results, status) {
 						if (status == google.maps.GeocoderStatus.OK) {
 							...
 							marker = new google.maps.Marker({
 								...
 								title : '선택쓰', // 마커에 마우스 포인트를 갖다댔을 때 뜨는 타이틀
