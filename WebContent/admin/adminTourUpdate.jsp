					...
					<div class="grids">
						...
						<div class="panel panel-widget forms-panel">
							<div class="progressbar-heading general-heading">
								<h4><b><img src="../custom/image/bar.png" width="7"/>&nbsp; ${tour_name} 수정 </b></h4>
							</div>
							<div class="forms">
								<h3 class="title1"></h3>
								<div class="form-three widget-shadow">
									<form name="tourForm" class="form-horizontal" action="adminTourUpdatePro.do?tour_type=${tour_type }" method="post">
										<div class="form-group">
											<label for="focusedinput" class="col-sm-2 control-label">도시</label>
											<div class="col-sm-5"> 
												<input type="text" class="form-control1" value="${tour.code_city }" disabled>
												<input type="hidden" class="form-control1" name="code_city" id="code_city" value="${tour.code_city }">
												<p class="help-block" id="error-city"></p>
											</div>
										</div>
										<div class="form-group">
											<label for="focusedinput" class="col-sm-2 control-label">코드</label>
											<div class="col-sm-5"> 
												<input type="text" class="form-control1" value="${tour.code }" disabled>
												<input type="hidden" class="form-control1" name="code" id="code" value="${tour.code}">
												<p class="help-block" id="error-code"></p>
											</div>
										</div>
										<div class="form-group">
											<label for="focusedinput" class="col-sm-2 control-label">${tour_name} 이름</label>
											<div class="col-sm-5">
												<input type="text" class="form-control1" name="name" id="name" value="${tour.name}" placeholder="한글로 입력하세요">
										...
										<div class="form-group">
											...
											<div class="col-sm-5">
												<input type="text" class="form-control1" name="address" id="address" value="${tour.address}" placeholder="한글로 입력하세요">
											...
											<div class="col-sm-5">
												<select name="icon" id="icon" class="form-control1">
													...
													<c:forEach var="icon" items="${iconList}">
														<c:if test="${fn:split(icon,'.')[0] == tour.icon}">
															<option value="${fn:split(icon,'.')[0]}" selected> ${fn:split(icon,".")[0]}</option>
														</c:if>
														<c:if test="${fn:split(icon,'.')[0] != tour.icon}">
										...
										<div class="form-group">
											...
											<div class="col-sm-8"><textarea name="description" id="description" cols="50" rows="4" class="form-control1" style="height:auto">${tour.description }</textarea></div>
										</div>
										<div class="btnlist">
									  		<input type="reset" class="btn col-5" onclick="hideErrorMasseage()" value="초기화">
									  		<input type="button" class="btn col-11" onclick="insertTourCheck()" value="수정">
...
$(function() {
	...
	if($('#icon option:selected').val()!="choose") {
		$('#iconImg').attr('src',"../custom/image/icon/"+$('#icon option:selected').val()+".png");
		$('#iconImg').attr('height','50px');$('#iconImg').attr('width','50px');
	}
