					...
					<div class="row"><!-- 도시리스트 선택과 버튼들을 정렬하기 위한 row -->
						...
						<div class="col-sm-5 col-sm-offset-1 col-md-5 col-md-offset-1">  
				  			<div class="row" id="btnrow"><!-- : 추가,삭제,수정버튼 정렬을 위한 row -->
						  		<div class="col-sm-4 col-sm-offset-4"><button type="button" class="btn btn-block" onclick="location.href='adminTourCityInsert.do'">추가</button></div>
							...
							<c:if test="${count!=0 }">
								<div class="row paging">
									<c:if test ="${pageUtil.startPage>pageUtil.blockUnit }">
										<a href="../admin/adminTourCity.do?currentPage=${pageUtil.startPage-1 }">[이전]</a>
									</c:if>
									<c:forEach var="num" begin="${pageUtil.startPage }" end="${pageUtil.endPage }">
										...
										<c:if test="${pageUtil.currentPage !=num}">
											<a href="../admin/adminTourCity.do?currentPage=${num }">${num }</a>
										</c:if>
									</c:forEach>
									<c:if test ="${pageUtil.endPage<pageUtil.totalPage }">
										<a href="../admin/adminTourCity.do?currentPage=${pageUtil.endPage+1 }">[다음]</a>
									</c:if>
								</div>
							</c:if>
						</div><!-- /.w3l-table-info -->
...
function goDetail(){
	$('#table tbody tr').bind({
		...
		click:function(){
			location.href="adminTourCityDetail.do?code_city="+$(this).find('td:eq(1)').html();
		}
	})
} 
