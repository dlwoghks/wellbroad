		...
		<div class="sub" id="sub">
			<div class="sub-menu"><!-- 왼쪽 메뉴 -->
				<ul class="nav navbar-nav">
					...
					<li><a href="crawler.do">CRAWLER</a></li>
				</ul><!-- /.nav -->
			</div><!-- /.sub-menu -->
		</div><!-- /#sub /.sub -->
	</div><!-- .main -->
	...
	<div class="sub-main">
		...
		<div class="article">
			<table>
				<tr>
					<div class="write">
						<c:if test="${adminId!=null}"><!-- session 키 sUserId가 로00.그인 되어있다면 -->
				...
				<table border="1" class="tablecontent1">
					...
					<c:forEach var="article" items="${articleList}">
						<tr height="30">
							...
							<td id="titlecolor">
								<a href="noticeContent.do?num=${article.num}&pageNum=${pgList.currentPage}">${article.title}</a>
							...
							<td style="text-align:center">${article.admin_id}</td>
					...
					<c:if test="${pgList.startPage>pgList.blockSize}">
						<li>
							<div class="pagination-next">
								<a href="noticeList.do?pageNum=${pgList.startPage-pgList.blockSize}&search=${search}&searchtext=${searchtext}"><<</a>
							</div>
						</li>
					</c:if>
		...
		<form name="test" action="noticeList.do" id="searchform" >
	...
	if ('<%=session.getAttribute("adminId")%>' != 'null') {
		$.ajax({url:"../admin/side.jsp",
			success:function(result) {
				$('.main #side').html(result);
				$('.main #side').css('overflow-y','scroll');
