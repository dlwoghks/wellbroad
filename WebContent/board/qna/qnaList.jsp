			...
			<section class="section-background">
				<div class="container">
					<h2 class="page-header0">
						<div class="notice-h"><b>QnA</b></div>
					</h2><!-- Board클릭시 보이는 타이틀 -->
	...
	<div class="sub-main">
		...
		<div class="article">	
   			<table>
				<tr>
					<div class="write">
						<a href="qnaWriteForm.do" style="float:left;">글쓰기</a>
					</div>
				</tr>
			</table>
					...
					<c:forEach var="article" items="${articleList}">
						<tr height="30">
							...
							<td id="titlecolor" align="left" >
								...
								<a href="qnaContent.do?num=${article.num}&pageNum=${pgList.currentPage}">${article.title}</a>
							</td>
							<td style="text-align:center">
								<fmt:formatDate value="${article.day}" timeStyle="medium" pattern="yy.MM.dd(hh:mm)" />
							</td>
							<td style="text-align:center">${article.id}</td>
							<td style="text-align:center">${article.mail}</td>
						</tr>
					</c:forEach>
				</table>
			</c:if>
				...
				<ul class="pagination">
					<c:if test="${pgList.startPage>pgList.blockSize}">
						<li><div class="pagination-next">
							<a href="qnaList.do?pageNum=${pgList.startPage-pgList.blockSize}&search=${search}&searchtext=${searchtext}"><<</a>
						</div>
					</c:if>
					<c:forEach var="i" begin="${pgList.startPage}" end="${pgList.endPage}">
						<a href="qnaList.do?pageNum=${i}&search=${search}&searchtext=${searchtext}">
					...
					<c:if test="${pgList.endPage<pgList.pageCount}">
						<li><a href="qnaList.do?pageNum=${pgList.startPage+pgList.blockSize}&search=${search}
		...
		<form name="test" action="qnaList.do" id="searchform">
